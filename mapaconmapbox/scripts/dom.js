...
function drawRoute(map, routeMapObject) {
  if (!map.isStyleLoaded()){
  ...
  function addRoute() {
    map.addSource(routeMapObject['name'], {
      'type': 'geojson',
      'data': {
        'type': 'Feature',
        'properties': {},
        'geometry': {
          'type': 'LineString',
          'coordinates': routeMapObject['coordinates']
        }
      }
    });
    addLayerRoute()
  ...
  function addLayerRoute() {
    map.addLayer({
      'id': 'route-'+routeMapObject['name'],
      'type': 'line',
      'source': routeMapObject['name'],
      'layout': {
        'line-join': 'round',
        'line-cap': 'round'
      },
      'paint': {
        'line-color': routeMapObject.stroke,
        'line-width': routeMapObject.strokeWidth
      }
    });
  }
